...
export function Sidebar({ activeTab, setActiveTab, onProfile, onSettings, isVisible }: SidebarProps) {
  ...
  return (
    <motion.aside 
      ...
      initial={false}
      animate={{ 
        width: isVisible ? (window.innerWidth < 768 ? '100vw' : '64px') : 0,
        opacity: isVisible ? 1 : 0,
        x: isVisible ? 0 : -20
      }}
      transition={{ duration: 0.3, ease: "easeInOut" }}
      className={`fixed bottom-0 left-0 right-0 h-16 md:h-screen border-t md:border-t-0 md:border-r border-white/5 bg-studio-bg flex md:flex-col items-center py-0 md:py-6 px-4 md:px-0 gap-8 z-[60] overflow-hidden ${!isVisible ? 'pointer-events-none' : ''}`}
    >
      ...
      <nav className="flex flex-1 md:flex-col items-center justify-around md:justify-start gap-4 md:gap-8 w-full md:w-auto">
        {menuItems.map((item) => (
          <button
            key={item.id}
      ...
      <div className="flex md:flex-col items-center gap-4 md:gap-6 mt-auto">
        {bottomItems.map((item) => (
          <button
            key={item.id}
            onClick={item.onClick}
            className="group relative p-3 text-slate-600 hover:text-slate-300 transition-colors rounded-xl"
          >
            <item.icon size={22} className="md:w-[24px] md:h-[24px]" />
            <span className="hidden md:block absolute left-full ml-4 px-2 py-1 bg-studio-surface text-white text-[10px] rounded opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50 shadow-xl border border-white/5">
              {item.label}
