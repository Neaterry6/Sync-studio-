...
export function Toolbar({ onToggleSidebar, onToggleLibrary, showLibrary, onInstallApp }: ToolbarProps) {
  ...
  return (
    <header id="toolbar-main" className="min-h-14 border-b border-white/5 bg-studio-bg/95 flex flex-wrap sm:flex-nowrap items-center justify-between gap-x-2 gap-y-1 px-2.5 sm:px-4 md:px-6 py-2 z-20">
      <input 
        type="file" 
        ref={fileInputRef} 
        className="hidden" 
      ...
      <div className="flex flex-1 items-center gap-1.5 sm:gap-3 min-w-0">
        ...
        <div className="h-4 w-px bg-white/5 mx-0.5 hidden min-[390px]:block" />
        <button className="hidden min-[390px]:grid p-2 hover:bg-studio-hover rounded-full text-slate-400 transition-colors">
          <SkipBack size={20} />
        </button>
        <div className="hidden min-[430px]:block font-mono text-xs text-slate-500 tabular-nums">00:00.5</div>
      </div>

      <div className="order-3 w-full justify-center sm:order-none sm:w-auto flex items-center bg-studio-surface/80 border border-white/5 p-1 rounded-xl gap-1">
        <div className="flex flex-1 sm:flex-none items-center justify-center px-2.5 sm:px-3 py-1.5 gap-2 border-r border-white/5 group relative">
        ...
        <div className="flex flex-1 sm:flex-none items-center justify-center px-2.5 sm:px-3 py-1.5 gap-2 group relative">
      ...
      <div className="flex items-center justify-end gap-1.5 sm:gap-3">
        {onInstallApp && (
          <button
            ...
            className="flex items-center gap-2 rounded-full bg-studio-primary px-3 py-2 text-xs font-bold text-white shadow-md shadow-black/20 transition-all hover:bg-studio-primary-hover active:scale-95"
